...
# [수정] 화면 흔들림 효과를 위해 모든 그림을 그릴 가상의 도화지(canvas) 생성
    ...
    while True:
        ...
        for knife in knives:
            ...
            if invincible == 0 and player_hitbox.colliderect(knife_hitbox):
                lives -= 1
                invincible = 90
                shake_timer = 6  # [추가] 피격 시 6프레임(약 0.1초) 동안 화면 흔들림 발생
        ...
        # --- 그리기 과정 ---
        # 1. 메인 화면(screen) 대신 가상의 도화지(canvas)에 모든 것을 그립니다.
        canvas.fill(BLACK)
        ...
        shake_x, shake_y = 0, 0
        if shake_timer > 0:
            # -8 ~ 8 픽셀 사이로 랜덤하게 상하좌우 흔들림
            shake_x = random.randint(-8, 8)
            shake_y = random.randint(-8, 8)
            shake_timer -= 1

        # 3. 완성된 도화지를 실제 메인 화면에 덮어씌웁니다 (흔들림 좌표 적용)
        screen.fill(BLACK) # 화면 밖 흔들림으로 인해 생기는 잔상 제거
